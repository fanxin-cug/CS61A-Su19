        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.is_container:
                    if not self.ant.can_contain(insect):
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        ant=self.place.ant
        if not ant or not ant.blocks_path:
            return False
        return True
        # END Problem 7
    ...
    def action(self, colony):
        ...
        if self.is_backwards:
            ...
            elif self.armor > 0 and destination is not colony.hive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        self.min_place=self.place
        self.max_place=self.place
        while self.max_place.entrance!=hive:
            self.max_place=self.max_place.entrance
        p = self.place
        while p!=self.max_place and p.bees==[]:
            p=p.entrance
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.max_range):
            if self.max_place.entrance==hive:
                break
            self.max_place=self.max_place.entrance
        p=self.min_place
        while p!=self.max_place and p.bees==[]:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.min_range):
            if self.min_place.entrance==hive:
                return
            self.min_place=self.min_place.entrance
        while self.max_place.entrance!=hive:
            self.max_place=self.max_place.entrance
        p = self.min_place
        while p!=self.max_place and p.bees==[]:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_truequeen:
            ...
            while p:
                if p.ant:
                    if hasattr(p.ant, 'damage') and not p.ant.damage_doubled:
                        p.ant.damage *= 2
                        p.ant.damage_doubled = True
                    if p.ant.is_container:
                        contained_ant=p.ant.contained_ant
                        if contained_ant and hasattr(contained_ant, 'damage') and not contained_ant.damage_doubled:
                            contained_ant.damage*=2
                            contained_ant.damage_doubled=True
                p=p.exit
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    if effect.__name__=='make_slow':
        if bee.action_slow==0:
            bee.action = effect(bee.action, bee)
        bee.action_slow += duration
    if effect.__name__=='make_scare':
        bee.action_scare=duration
        bee.action = effect(bee.action, bee)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        while p!=hive:
            if p.ant:
                if p.ant.is_container and p.ant.contained_ant:
                    d[p.ant.contained_ant]=dis
                d[p.ant] = dis
            if p.bees:
                for bee in p.bees:
                    d[bee]=dis
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        #return 0
        weakened=0.2*distance
        damage_down=0.05*self.insects_shot
        damage=self.damage-weakened-damage_down
        if damage>0:
            return damage
        else:
            return 0
