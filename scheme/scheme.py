...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals)!=len(vals):
            raise SchemeError('Num of arguments does not match with formal paramenters.')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('arguments are not valid: {0}'.format(python_args))
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    for _ in range(len(bindings)):
        ...
        val=scheme_eval(bindings.first.second.first,env)
        formals=Pair(formal,formals)
        vals=Pair(val,vals)
        bindings=bindings.second
    new_env=env.make_child_frame(formals,vals)
    return new_env
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        check_formals(formals)
        body=expressions.second
        env.define(target,MacroProcedure(formals,body,env))
        return target
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
