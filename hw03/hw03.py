...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    side_l,side_r=left(m),right(m)
    end_l,end_r=end(side_l),end(side_r)
    if is_weight(end_l) and is_weight(end_r):
        return size(end_l)*length(side_l)==size(end_r)*length(side_r)
    else:
        total_l,total_r=total_weight(end_l),total_weight(end_r)
        if total_l*length(side_l)!=total_r*length(side_r):
            return False
        else:
            lst=[]
            if is_mobile(end_l):
...
def totals_tree(m):
    ...
    if is_weight(end_l) and is_weight(end_r):
        return tree(total_weight(m),[tree(size(end_l)),tree(size(end_r))])
    else:
        lst=[]
        if is_mobile(end_l):
            lst+=[totals_tree(end_l)]
        else:
            lst+=[tree(size(end_l))]
        if is_mobile(end_r):
            lst+=[totals_tree(end_r)]
        else:
            lst+=[tree(size(end_r))]
